...
```text
Датасет содержит 900+ патчей, собранных для 45 независимых станций геологического пробоотбора с использованием данных ГЛБО,
снятых в разные годы на одном и том же полигоне 
```
...
## Структура проекта
В данном проекте представлены основные результаты работы, выложена лучшая модель для предсказания гранулометрических свойств донного грунта по результатам экспериментов (R²≈0.7)
