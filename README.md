...
Рабочий пайплайн для предсказания средневзвешенного размера зерна донных осадков `Mz_phi` по данным гидролокации бокового обзора (ГЛБО) и батиметрии. Итоговая модель — `RandomForestRegressor` с инженерными признаками, рассчитанными из локальных патчей растров.

Проект включает только финальную рабочую RF-линию: подготовка датасета `.npz`, обучение моделей RF, инференс по GeoTIFF и проверка предсказаний в точках станций.
...
├── pyproject.toml
├── scripts/
│   ├── create_dataset.py       # станции + GeoTIFF → обучающий NPZ
│   ├── inspect_npz.py          # проверка NPZ и визуализация случайных патчей
│   ├── train_rf.py             # обучение финальных RandomForest-моделей
│   └── predict_geotiff.py      # GeoTIFF ГЛБО + GeoTIFF батиметрии → карта Mz_phi
└── src/seabed_rf/
...
Рекомендуемая структура данных при запуске:

```text
project_root/
├── Dataset/
│   ├── patches_with_features_dataset.npz
│   └── rf_final_training_outputs/
├── test_data/
│   ├── 0_Sonar_data_Nilma.tif
│   ├── 1_Bathy_data_Nilma.tif
│   ├── Nilma_stations.txt
│   ├── 2_Sonar_data_Rugozero.tif
│   ├── 2_Bathy_data_Rugozero.tif
│   └── Rugozero_stations.txt
└── seabed-rf-pipeline/
```
...
Поддерживаются табуляция, пробелы, запятая и точка с запятой. Колонка `Mz` — целевая переменная `Mz_phi` в точке станции. Для инференса она нужна только для оценки ошибки в контрольных точках.
...
## 5. Обучение финальной RF-модели
...
4. Исключает технические признаки из predictors:

   ```text
   valid_mask, valid_ratio, n_patches, n_rasters, shift_*, source_raster
   ```

5. Формирует три представления данных:
...
6. Оценивает модели через repeated stratified CV:
...
### 5.2. Почему для GeoTIFF-инференса используется `RF_patch_level_weighted`

Для карты предсказаний модель применяется к локальному скользящему окну:

```text
один локальный патч → признаки → одно предсказание Mz_phi
```

Поэтому наиболее совместима с картированием модель `RF_patch_level_weighted`. Она обучается на отдельных патчах, но использует веса:

```text
sample_weight = 1 / число патчей станции
```

Так станции с большим числом патчей и растров не доминируют в обучении.

`RF_station_level` может показывать лучшее качество по станциям, но она опирается на агрегацию нескольких патчей/растров одной станции и менее напрямую переносится на пиксельную карту.

### 5.3. Результаты обучения
...
rf_final_training_outputs/
├── FINAL_SUMMARY.csv
├── cv_metrics_repeated.csv
├── repeated_oof_predictions_raw.csv
├── repeated_oof_predictions_by_station.csv
├── patch_level_features.csv
├── station_raster_level_features.csv
├── station_level_features.csv
├── sonar_normalization_stats.json
├── inference_config.json
├── figures/
│   ├── RF_patch_level_weighted_station_oof_scatter.png
│   ├── RF_station_raster_level_station_oof_scatter.png
│   └── RF_station_level_station_oof_scatter.png
└── models/
    ├── RF_patch_level_weighted.joblib
    ├── RF_patch_level_weighted_feature_columns.json
    ├── RF_station_raster_level.joblib
    ├── RF_station_raster_level_feature_columns.json
    ├── RF_station_level.joblib
    └── RF_station_level_feature_columns.json
```

`FINAL_SUMMARY.csv` содержит итоговые station-level метрики по каждому эксперименту:

- `MAE` — средняя абсолютная ошибка в φ-шкале;
...
## 8. Методологические замечания

1. **RandomForest не использует изображения напрямую.** Он обучается на признаках, рассчитанных из патчей: статистиках, текстурах и морфометрических характеристиках.
2. **Классическая image-аугментация не применяется.** Вместо неё используются естественные повторные наблюдения: разные проходы ГЛБО и сдвиги патча вокруг станции.
3. **Для оценки качества используется station-level подход.** Все патчи одной станции должны попадать только в один fold.
4. **Для картирования используется patch-level модель.** Она наиболее совместима с применением к скользящему окну по GeoTIFF.
5. **Технические признаки не используются как predictors.** Маски, число патчей, имя растра и сдвиги сохраняются для диагностики, но исключаются из обучения, чтобы повысить переносимость.
6. **Инференс выполняется только на пересечении валидных областей ГЛБО и батиметрии.** Это предотвращает предсказания на участках, где есть только батиметрия, но отсутствует реальный сигнал ГЛБО.

---

## 9. Типовой полный сценарий
...
## 10. Что делать, если качество на внешнем полигоне хуже

Проверьте:

1. совпадает ли шкала и обработка ГЛБО;
2. корректно ли задана маска nodata/белого фона;
3. попадают ли станции в область пересечения ГЛБО и батиметрии;
4. лежит ли внешний полигон в том же диапазоне глубин, Mz_phi и морфологии;
5. не используются ли технические признаки при обучении;
6. достаточно ли похожи разрешение и размер патча в метрах.

Если внешний полигон сильно отличается, модель может потребовать дообучения или калибровки на нескольких станциях нового полигона.
